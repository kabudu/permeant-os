...
\begin{enumerate}[leftmargin=*]
  ...
  \item We report a real MLX-to-vLLM end-to-end validation with exact 128-token decode fidelity.
\end{enumerate}
...
The source host ran MLX with \texttt{Qwen/Qwen2.5-0.5B-Instruct}. The target host was an AWS \texttt{g4dn.xlarge} instance running vLLM \texttt{0.23.0} with the same model. PermeantOS exported a 1920-token source prompt/cache, streamed the encrypted migration over production \texttt{wss://}/mTLS transport, registered target KV blocks, seeded vLLM prefix-cache metadata, and generated a post-migration continuation from the migrated prefix.

The target context length was kept below the 2048-token model window to leave room for a 128-token validation continuation. An earlier run using a 2032-token source prefix produced an apparent mismatch after 11 tokens; analysis showed the target had reached its context limit and stopped early.
...
Metric & Result \\
\midrule
Run ID & \texttt{20260621-052744} \\
Manifest & \texttt{migration-20260621-053602-9938-manifest.json} \\
...
Transfer quantization & experimental \texttt{qatq} \\
Agent Memory Graph binding & 27-node complex package aligned and resumed on target \\
Layer count & 24 \\
Hash validation & Passed \\
Slot-probe max key diff & \texttt{0.006696999999999065} \\
Slot-probe max value diff & \texttt{0.000558149999999813} \\
vLLM prefix-cache seeded blocks & 16 \\
Source vs. post-migration decode & Exact at 16, 32, 64, and 128 tokens \\
Target baseline vs. post-migration decode & Exact at 16, 32, 64, and 128 tokens \\
Reverse runtime import & vLLM export API to MLX origin continuation \\
Target-side agent activity resume & Completed read-only and approved publish work \\
Post-resume proof hash & \texttt{sha256:b066a1dba9ed250eb54e1344c8d0092d8ad2d90dfe68bbfc1a0c740d18b6969c} \\
Origin return-home continuation & Verified AWS evidence and wrote origin continuation \\
Round-trip proof hash & \texttt{sha256:052add6058521a13902515f759499b1350d5be4055d070d4e5428a9df0adb36d} \\
...
The latest QATQ long-horizon run reduced transferred bytes to 6,294,528 from a 47,185,920-byte uncompressed KV payload, a compression ratio of 0.1333984375. QATQ is not numerically lossless: sampled slot probes reported maximum key/value deltas of 0.006696999999999065 and 0.000558149999999813 respectively in the long-horizon round-trip run. The validation claim is therefore bounded lossy transfer with exact observed decode fidelity over the configured 128-token continuation, reverse runtime import, plus target-side and origin return-home Agent Memory Graph activity continuation.
...
The current system is a research preview. The real-runtime validation covers one model family, one source runtime, one target runtime, complex Agent Memory Graph transaction binding, target-side resume, reverse vLLM-to-MLX runtime-state import, origin return-home graph/artifact continuation, and a 128-token continuation horizon. The vLLM adapter uses internal implementation details that may change across vLLM versions. Cloud-host validation is currently dominated by cold installation and warmup costs unless prewarmed images are used. Durable target-side graph session storage, broader runtime coverage, broader reverse runtime coverage, and additional model-family validation remain future work.

\section{Conclusion}

USXF and PermeantOS provide a concrete path toward portable, verifiable AI agent state. The current prototype demonstrates that a real MLX source cache and complex Agent Memory Graph package can be migrated to a real vLLM target, written into target KV storage, attached through prefix-cache metadata, decoded with exact 128-token fidelity, exported back through a live target reverse runtime API, imported into MLX as target-advanced runtime state, resumed on the AWS target with new policy-governed agent activity, returned to the origin as graph/artifact evidence, and continued on the origin from that remote proof. This result supports the broader thesis that AI agent state should be treated as migratable infrastructure rather than runtime-local ephemera.
