...
pub fn canonical_to_vllm_block_key(canonical: &Tensor, block_size: usize) -> Result<Tensor> {
    ...
    Ok(Tensor::new(
        vllm_data,
        vec![num_blocks, num_kv_heads, head_dim, block_size],
    ))
}
...
pub fn vllm_block_to_canonical_key(
    ...
    if vllm.shape.len() != 4 {
        bail!(
            "vLLM block key must have 4 dimensions [num_blocks, num_kv_heads, head_dim, block_size]"
        );
    }
    ...
    if num_blocks < expected_blocks {
        bail!(
            "vLLM tensor doesn't contain enough blocks for seq_len {}",
            seq_len
        );
    }
...
pub fn canonical_to_vllm_block_value(canonical: &Tensor, block_size: usize) -> Result<Tensor> {
    if canonical.shape.len() != 3 {
    ...
    Ok(Tensor::new(
        vllm_data,
        vec![num_blocks, num_kv_heads, block_size, head_dim],
    ))
}
...
pub fn vllm_block_to_canonical_value(
    ...
    if vllm.shape.len() != 4 {
        bail!(
            "vLLM block value must have 4 dimensions [num_blocks, num_kv_heads, block_size, head_dim]"
        );
    }
    let num_blocks = vllm.shape[0];
    let num_kv_heads = vllm.shape[1];
